...
A streak of one would be one hit followed by a miss. A streak of 0 is a miss followed by another miss.
...
The data is right skewed, the median value is 0 and the mean is .76, and longest streak is 4.
    
    
```{r}

mosaic::favstats(kobe_streak$length)
...
```{r set-seed}
set.seed(3939)                 # make sure to change the seed
...
```{r}
barplot(table(sim_streak), col = "purple")
...
If we were right now, it would not change at all thanks to the seed. If were to remove that, we could run this simulation over and over and get slightly different results every time. However, we should expect the same distribution, and overall very similar values each time regardless of how many times we run the simulation. 
...
The independent shooter has the same distribution of streak lengths as Kobe Bryant. Based on this info alone, we can see that Kobe Bryant's shots are not independent, but it would be prudent to look at more data in order to get the best understanding possible.  
* * *

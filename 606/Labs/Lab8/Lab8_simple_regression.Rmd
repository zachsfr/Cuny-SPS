...
options(digits = 2)
 ...
 M <- select(hfi,pf_score,pf_ss_homicide,pf_religion,pf_identity_parental_divorce,ef_legal_military,pf_ss_disappearances_violent,pf_ss_women_missing,pf_ss
) %>%
  drop_na() %>%
cor()
corrplot(M, type="upper", order="hclust",
         col=brewer.pal(n=8, name="RdYlBu"), insig = "p-value",p.mat = M, method = "square")


ggplot(data=hfi, aes(x= pf_ss_homicide,y =pf_score)) + geom_point( na.rm =T)  + geom_smooth(method = lm,  na.rm =T)  
...
|   It appears that, personal freedom and homicide have a positive correlation of about .25. 
...
# This will only work interactively (i.e. will not show in the knitted document)
 ...
 sub <- select(hfi,pf_expression_control,pf_score) %>%
  drop_na()
DATA606::plot_ss(x = sub$pf_expression_control,sub$pf_score)
```
...
DATA606::plot_ss( x = sub$pf_expression_control, y = sub$pf_score, showSquares = TRUE)
```
...
|   I was able to get a sum of squares of 956. 
.
...
5.  Fit a new model that uses `pf_expression_control` to predict `hf_score`, or
    ...
    context of the relationsh ip between human freedom and the amount of political
...
The last squares regression line for our second linear model is

\[
  \hat{y} = 5.15369 + 0.34986 \times pf\_expression\_control
\]


The $R^2$ value for this model is 57.70%
...
|   This value fits within our line so it would not be an over or under estimate. Our value would be a pf_score of around 7.5. And as shown below, if we use the numbers from our lm coefficents above, we calculate that same value. 
...
|   We notice that the values appear to be normally distributed around 0, veering more to one side on the far ends a little bit. There is no obvious and clear pattern occuring. 
...
|   It appears to be met. The values appear roughly normally distributed. 
</br>
...
9.  Based on the residuals vs. fitted plot, does the constant variability condition 
    appear to be met?
| It would appear that this condition is met.     
...
m3 <- lm(ef_regulation ~ pf_ss_disappearances_violent, data = hfi)
...
```{r}
ggplot(data=hfi, aes(x= pf_ss_disappearances_violent,y =ef_regulation)) + geom_point( na.rm =T)  + geom_smooth(method = lm,  na.rm =T)  
...
| It appears that, as economic freedom regulation increases, the number of violent conflict increases. However, upon further inspection of the data, our variable pf_ss_disappearances_violent appears to be heavily left skewed. We need further analysis before making any hypothesis. 

    Violent conflicts


```{r}
mosaic::favstats(hfi$pf_ss_disappearances_violent)
...
```{r}
ggplot(data = hfi, aes(x = pf_ss_disappearances_violent)) +
  geom_histogram() 
```

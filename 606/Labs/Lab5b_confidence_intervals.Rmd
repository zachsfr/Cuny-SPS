...
```
|
|   61 percent of adults in this sample think climate change affects their local community.
|
|
1.  Would you expect another student's sample proportion to be identical to yours? 
    Would you expect it to be similar? Why or why not?
    
|   So long as they used the same seed, it will be exactly the same. If they used a different seed, they will have slightly different values but a similar overall proportion, as the sample will roughly represent the overall populations value.
...
|   95% confidence means that we are 95% certain that within our range of values exists the true mean of the population data.
...
1.  Each student should have gotten a slightly different confidence interval. What 
    ...
    mean? Why?
|   95%, meaning only 5% of the students should be incorrect, or in other words, 1 in 20 of the students.
...
```{r, echo=F}
knitr::include_graphics("https://i.gyazo.com/2b514c2c00168e6218995d981c802980.png")

```
    
|   As shown above, all but one of my confidence intervals contains the true population proportion. This makes leaves us with a 98% success rate and a 2% failure rate, which means we are achieving better results than we would expect. 
* * *
...
```{r, echo=F}
knitr::include_graphics("https://i.gyazo.com/df767035f0d28d0acf61e0c2c839ea94.png")

```

|   I used a 75% confidence interval this time. The intervals were far narrower than  in the 95% confidence interval. My result can be seen above. 
...
```{r, echo=F}
knitr::include_graphics("https://i.gyazo.com/28fb6374a489b58ca2a004a0ea974a30.png")

```


|   As shown above, using a 85% confidence interval, we managed to have 11 incorrect intervals, leaving us at a percentage of 22% incorrect and 88% correct. Meaning, we were able to have more accuracy than expected.
...
Using a 10% confidence interval, I expect the intervals to be extremely narrow, much shorter than all the previous intervals I calculated

```{r, echo=F}
knitr::include_graphics("https://i.gyazo.com/526511422cf11d0ac7969b1335168495.png")

```

|   And sure enough, the intervals are extremely narrow. We have a success rate of 2 out of 50. In other words, we were correct 4% of the time and incorrect 96% of the time, leaving us below our expected outcome of 10% correct and 90% incorrect. 
...
| Since the app uses random samples every time it's tough to pin down however, it appears that as sample size increases, the width decreases and as the sample size decrease, the width will have an equal and opposite reaction of increasing. 
...
| As the same size increases, the width of the intervals begin to decrease more and more. 

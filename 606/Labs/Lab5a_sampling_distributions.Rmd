...
samp1 %>%
  count(scientist_work) %>%
  mutate(samp_p = n /sum(n))
```

|   I disocvered through this lab that dpylr's sample function will not respect the seed unless it's placed together in the same chunk.  However, calling the seed once seems to work properly when knitting the file, as the output in the knitted file is the same each time. Using the  knitted files values, the distribution in the sample, while not the same as the overall populations, are very similar. We have .74 for benefits and .26 for doesn't benefit. 
...
|   I would expect them to match, as we are all using the same pre-selected seed. 
...
| I would assume that the closer we approach the original population size, the more often our sample distribution will match our original distribution.
...
|   We have 15,000 elements inside sample_props50. It has a center at .2 with a median of .2 and a mean of .199, very close to .2. It appears to be a normal distribution with a clean standard deviation and healthy set of quantiles. 

```{r}
mosaic::favstats(sample_props50$p_hat)
```
...
sample_props10 <- global_monitor %>%
                    ...
                    filter(scientist_work == "Doesn't benefit")
sample_props10
mosaic::favstats(sample_props10$p_hat)
```
|   The number of observations appears to vary from between 44-50 with the seeded output having 22 in each group. Each observation represents a sample population of doesn't benefit group.  
...
|   Each observation is a random sample from the original population. As the sample size increases, all those values become further standardized around the original value of overall population data. In doing so, the error also decreases. As we increase the number of simulation, the same change will occur. 
...
ggplot(data = sample_props15, aes(x = p_hat)) +
  ...
  labs(x = "Sample population (Benefits)", y = "Count",
    title = "Sampling distribution of the sample population",
...
```{r}
mosaic::favstats(sample_props15$p_hat)
ggpubr::ggqqplot(sample_props15$p_hat)
DATA606::qqnormsim(sample_props15$p_hat)

```

|   The distributions appearance is is heavily influenced by bin size.  As we see in the first histogram, the distribution appears somewhat normal but becomes more skewed in look the larger the bins become. The mean and median are nearly the same, but the quantiles are very wonky with with Q1 and Q3 existing less than one SD away from the mean. Overall I would like to call this normally distributed around .8.
...
sample_props150
mosaic::favstats(sample_props150$p_hat)
...
The values are far more normal in this sample. We have a clearly normal distribution with a clear center at .8. It makes it clear that the true proportion is likely the same value, .8.
...
|   The second sample we created with 100 more in it had a smaller standard deviation, smaller IQR, and was overall spread much less which led to a much clearer understanding of what was occurring within our data. In the first sample we created, with a sample size of 15, the standard deviation was so large that the quantiles weren't even one standard deviation away from the mean. That made it extremely hard to make sense of what was happening in our distribution

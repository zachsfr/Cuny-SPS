...
```{r}
 # proportion for people who have driven during the 30 day period

yrbss %>%
mutate(Helmet = ifelse(helmet_12m == "never",'no','yes'), drove = ifelse(str_detect(text_while_driving_30d,"[1-9]+"),'yes','no')) %>%
filter(!is.na(drove) &!is.na(Helmet) ) %>%
summarise(proportion = sum(drove=="yes" & Helmet=="no") /n() )

# Proportion of people who have driven every single day during the 30 days without a helmet 
yrbss %>%
mutate(Helmet = ifelse(helmet_12m == "never",'no','yes'), drove = ifelse(text_while_driving_30d == "30",'yes','no')) %>%
filter(!is.na(drove) &!is.na(Helmet) ) %>%
summarise(proportion = sum(drove=="yes" & Helmet=="no") /n() )
...
summarise(proportion = sum(text_ind=="yes") /n() )
```
...
|   We can find the margin of error by subtracting our confidence intervals from each other and diving that value by two.  Doing so, we get a margin of error of .006.
...
male <- yrbss %>%
  filter(gender=="male") %>% 
  mutate(healty_weight= weight <=70)
  
male %>%
  
specify(response = healty_weight, success = "TRUE") %>%
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "prop") %>%
  get_ci(level = 0.95)
...
white <-  yrbss %>%
  filter(race=="White") %>% 
  mutate(healty_seep= school_night_hours_sleep >=8 | school_night_hours_sleep == '10+')
  
white %>%
  
specify(response = healty_seep, success = "TRUE") %>%
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "prop") %>%
  get_ci(level = 0.95)
...
|   It appears that, as the  population proportion goes from  0 to .5, the margin of error likewise increases, reaching a peak at a population proportion of .5. From .5 onward, the margin of error continually decreases at the same rate, until it reaches 0 at a population proportion of 1. 
...
```{r, echo=F}
knitr::include_graphics("https://i.gyazo.com/22068b9106e74842430979f2f9ccafdd.png")

```

|   The distribution appears normal for these values. The center appears around .15 with other, smaller, peaks surrounding it.
...
| It appears that, as we go from 0 to .5, the values become more spread out. Then, once you pass .5, the values begin to become less spread again, eventually converging to one value.    
    
8.  Now also change $n$. How does $n$ appear to affect the distribution of $\hat{p}$?
| It appears that, the larger n becomes, the less spread out the values are as well.
* * *
...
tenPlus <- tenPlus %>%
  mutate(train_everiD = ifelse(strength_training_7d == "7", "yes", "no"))

tenPlus %>%
  specify(response = train_everiD, success = "yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "prop") %>%
  get_ci(level = 0.95)
...
| Ho: There is no convincing evidence that those who sleep 10+ hours per day are more
    likely to strength train every day of the week

| Ha: There is convincing evidence that those who sleep 10+ hours per day are more
    likely to strength train every day of the week
...
| At a significance level of .05, incorrectly rejecting the true null hypothesis should be 5%. 

...
knitr::include_graphics("~/CUNY SPS/606/Labs/Lab1/img/r-interface-2020.png")
```
...
source('~/CUNY SPS/606/Labs/Lab1/more/arbuthnot.r')
```
...
\paragraph{} The overall trend seen in the data is shown in the graph below. Overtime, the number of girls baptized has continually increased.  
...
\paragraph{} As seen in the graphs below, the proportion of boys overtime decreases in both. 
...
\paragraph{} Even the smallest total counts of the present data set are at the very minimum 153 times larger than the largest total counts of the arbuthnot data set.

```{r}
Minpres <- present %>%
  transmute(TotalCountP = boys + girls) %>%
  top_n(-10,TotalCountP) %>%
  arrange(TotalCountP)
  
  
topAr <- arbuthnot %>%
    transmute(TotalCountA = boys + girls) %>%
  top_n(10,TotalCountA)  %>%
  arrange(TotalCountA)


cbind(Minpres,topAr) %>%
  summarise(TotalCountP/TotalCountA)
...
\paragraph{} There is an overall greater proportion of boys than girls however, over time that ratio decreases in the same way as the Arbuthnot data.
    
```{r}
present %>%
  mutate(boy_ratio = boys / (boys+girls))  %>%
...
\paragraph{} The year with the greatest total count is shown below to be 1961.
    
    
```{r sample-arrange, eval=FALSE}
present %>%
  mutate(TotalCountP = boys + girls) %>%
  top_n(1,TotalCountP)
```

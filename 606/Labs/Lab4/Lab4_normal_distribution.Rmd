...
Looking at both the base R histograms and the ggplot graphs using different bin numbers, both distributions seem to be unimodal and right skewed.
```{r}
...
ggplot(dairy_queen,aes(cal_fat)) +
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    distribution?
Appears to be somewhat normal, but also clearly right skewed. How skewed is too skewed and how close to normal is close enough?
...
The first thing of note is that the Y values differ. In sim_norm the plot begins below y = 0 and ends before reaching  y = 600. In the original, it begins at around y = 0 and ends at nearly y = 700. The slope of the line at various points also vary greatly. Overall however, the shape is very similar. How similar is similar enough?
...
Using the qqnormsim and the ggqqplot in conjuction, we can see that the data is almost normal, with some values skewing at the upper end. 
...
Using both methods above, we can see that, similar to dairy queen, the values closely follow the normal distribution up until the upper end. At that point the values wildly veer off into being extreme.
...
Question One: What is the probability that a randomly chosen product from subway has 1,000 calories or more?
...
1 - pnorm(q = 1000, mean = s_mean, sd = s_sd)
...
Question Two: What is the chance that a randomly chosen product from McDonald's has less than or an equal amount of 900 grams of sodium

```{r}

m_mean <- mean(mcdonalds$sodium)
m_sd <- sd(mcdonalds$sodium)

1 - pnorm(q = 900, mean = m_mean, sd = m_sd)
...
```{r}
uniqe = unique(fastfood$restaurant)


for (i in uniqe) {
  print(i)
 print(qqnormsim(sample = sodium,data= filter(fastfood, restaurant == i)))
...
ggqqplot(taco$sodium)
ggqqplot(subway$sodium)

```

It appears that taco bell is the closest to normal amongst the restraunts for sodium. 
...
Most likely do to the variation in food products they offer. Some may be very similar, such as cheeseburgers, but the other products step up from a lower amount to a much higher amount very quickly. Going from salads, drinks, french fries, to a bacon cheeseburger, and you will be stepping up the sodium as you move along.
...
```{r}
qqnormsim(sample = total_carb,data= filter(fastfood, restaurant == "Taco Bell"))

 ggqqplot(taco$total_carb)

hist(taco$total_carb)   

ggplot(taco,aes(total_carb)) +
  geom_histogram(bins=55)

```
The qqplots show that the distribution for total carbohydrates in Taco Bell's menu to be mostly normal. However, the histograms make it tough to visualize. By bumping up the biun number in the ggplot model, we can somewhat see one major hump wtowars the middle values, whith some more exteme values towards the end. 

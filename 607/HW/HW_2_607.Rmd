...
\paragraph{} In addition, in order to properly quantity the respondents responses, we need to create a numeric column that represents each response. Doing so with dpylr's mutate and case_when functions make this very easy. We can change each response to the appropriate number, and make the non responses NA values. 
